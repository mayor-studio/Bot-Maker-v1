  ...
  async execute(interaction){
    if (interaction.isStringSelectMenu()) {
        if(interaction.customId == 'select_bot') {
          ...
          if(selected == "BuyRoles") {
            if(buyStatusDB.get(`roles`) === "off"){
          ...
          let rolesPrice = parseInt(prices.get(`roles_price_${interaction.guild.id}`))
          if(!rolesPrice) rolesPrice = 70;
...
userbalance = parseInt(usersdata.get(`balance_${interaction.user.id}_${interaction.guild.id}`))
        if(userbalance < rolesPrice) return interaction.reply({content:`**⛔ انت لا تمتلك الرصيد الكافي ، تحتاج الى \`${rolesPrice}\` عملات**` , ephemeral:true})
           const modal = new ModalBuilder()
          .setCustomId('BuyRoles_Modal')
       .setTitle('Make roles Bot');
